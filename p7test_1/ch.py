...
def main():
    #asmfilename="testpoint"+str(jjj)+".asm"
    ...
    for folder in filelist:
            for file in folder[2]:
                if(len(file.split("."))>1 and file.split(".")[1]=="asm"):
                    asmfilename=folder[0]+"\\"+file
                    print(asmfilename)
                    path=os.path.dirname(os.path.realpath(__file__))
                    os.chdir(path)
                    #subprocess.Popen(["java","-jar","marss.jar","a","db","nc","mc","CompactDataAtZero","dump","0x00004180-0x00005180","HexText","code_handler.txt",asmfilename],stdout=subprocess.DEVNULL).wait(10)
                    #subprocess.Popen(["java","-jar","marss.jar","db","nc","mc","CompactDataAtZero","100000","dump",".text","HexText","code.txt",asmfilename],stdout=subprocess.DEVNULL).wait(10)
                    os.system("java -jar marss.jar a db nc mc CompactDataAtZero dump 0x00004180-0x00005180 HexText code_handler.txt "+asmfilename)
                    os.system("java -jar marss.jar db nc mc CompactDataAtZero dump .text HexText code.txt >ans.txt "+asmfilename)
                    #copyfile("D:\\test\\code.txt","D:\\dhy\\code.txt")
                    #copyfile("D:\\test\\code_handler.txt","D:\\dhy\\code_handler.txt")
                    ...
                    '''
                    os.chdir("D:\dhy")
                    os.system("D:\dhy\mips.exe -nolog -tclbatch mips.tcl >dhy.txt")
                    with open("dhy.txt","r") as my:
                        lines=my.readlines()
                        if(len(lines)==0):
                    ...
                    x=os.system("fc D:\\test\\my.txt D:\\dhy\\dhy.txt")
                    ...
                    print("Wow,maybe you are correct on this file.")
    os._exit(0)
